...
1. 打开工作台，导入开启自动质量词的 V5 图片。
   - 预期结果：Base Prompt 标题下显示可点击的“NovelAI V5 Quality Tags Standard”提示和数量；对应自动 Tag 默认折叠，不出现在 Tag 列表中。
   - 实际结果：

2. 点击 V5 Quality Tags 提示，再将鼠标停留在任一展开的自动质量 Tag 上；随后再次点击提示。
   - 预期结果：第一次点击后自动 Tag 出现，提示箭头指向展开状态；悬浮信息明确显示来源为“NovelAI V5 Quality Tags · Standard”。第二次点击后自动 Tag 收起，用户 Tag 保持不变。
   - 实际结果：

3. 检查同一图片的 Base Undesired Content。
   - 预期结果：能够按照图片元数据和文本显示可点击的 Heavy、Light、Human Focus 或其他已确认的 UC 预设名称；预设 Tag 默认折叠，点击提示后显示，再次点击后收起。
   - 实际结果：
...
7. 导入 V4.5 Full 或 Curated 图片，重复检查 Base Prompt、自动 Tag 悬浮来源和两种“复制全部”结果。
   - 预期结果：界面显示正确的 V4.5 模型类型；自动质量词默认折叠且可通过提示展开；默认复制排除对应模型的自动质量词，显式“包含自动 Tag”复制保留完整原文。
   - 实际结果：

8. 导入关闭自动质量词、但手动包含 `very aesthetic, masterpiece, no text` 等相同词组的图片。
   - 预期结果：这些用户手写 Tag 不显示为已确认自动来源，也不会被默认折叠或从复制结果中删除。
   - 实际结果：
...
10. 分别在浅色和深色主题、最小窗口宽度下检查 Base Prompt、Base Undesired Content、Tag 来源样式和复制菜单。
    - 预期结果：提示文字可辨认，识别提示与下方 Tag 列表之间留有清晰间距；鼠标点击、Tab 聚焦及 Enter/空格均可切换展开状态；长提示可截断并通过标题说明查看，不遮挡操作按钮，Tag 与菜单没有溢出或错位。
    - 实际结果：

11. 在工作台打开回归图片 `772245e5-1309-481d-9877-a04851eac2ca.png`。
    - 预期结果：Base Prompt 显示“推断 NovelAI V4.5 Full Quality Tags Standard · 3”，三个对应 Tag 默认折叠并可点击提示展开；Base Undesired Content 显示“推断 NovelAI UC Heavy · 17”，即使 Heavy 前面还有用户输入的 `nsfw`，17 个预设 Tag 也默认折叠并可独立展开。
    - 实际结果：
...
- 总体结果：通过。用户于 2026-08-24 确认已完成 Windows 验证并要求归档。
- 发现的问题：无。
