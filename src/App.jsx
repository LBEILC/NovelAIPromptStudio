  ...
  return <aside className="library-panel">
    ...
    <div className="asset-list">
      {projects.map((project) => <button key={project.id} className={`asset-row ${project.id === activeId ? 'active' : ''}`} onClick={() => setActiveId(project.id)}>
        <img src={mediaUrl(project.thumbnail_path)} alt=""/>
        <span className="asset-copy"><strong>{project.name}</strong><small>{project.tags.length} tags · {relativeTime(project.updated_at)}</small></span>
...
function PreviewStage({ project, onCopy, onReveal, saveVersion, restoreVersion, activeVersion, setActiveVersion }) {
  return <section className="preview-column">
    <header className="topbar">
      ...
      <div className="top-actions">
        <button className="ghost" onClick={() => onReveal(project.image_path)} title="在访达中显示"><Icon name="folder"/></button>
    ...
    <div className="stage">
      <div className="image-mat">
        ...
        <div className="image-index">ASSET<br/><b>{String(project.tags.length).padStart(2, '0')}</b></div>
      </div>
...
function TagCard({ tag, index, translating, dragging, dropTarget, onTranslate, onChange, onDelete, onPointerStart, onPointerMove, onPointerEnd, onKeyboardMove }) {
  return <article data-tag-index={index} className={`tag-card ${dragging ? 'dragging' : ''} ${dropTarget ? 'drop-target' : ''}`}>
...
function TagsPanel({ project, updateProject, showToast }) {
  ...
  const translateTagIds = async (ids) => {
    const targets = project.tags.filter((tag) => ids.includes(tag.id)).slice(0, 50);
    ...
    updateProject({ ...project, tags: project.tags.map((tag) => translated.has(tag.id) ? { ...tag, translation: translated.get(tag.id) } : tag) });
    setAIStatus({ type: 'success', text: `${result.model} 已补齐 ${targets.length} 个译名` });
    showToast(`AI 已翻译 ${targets.length} 个 Tag`);
  };

  const missingTranslationIds = project.tags.filter((tag) => !tag.translation?.trim()).map((tag) => tag.id);
  const addTag = () => {
    if (!newTag.trim()) return;
    const created = parsePrompt(newTag)[0];
    updateProject({ ...project, tags: [...project.tags, created] });
    setNewTag('');
  };
  const updateTag = (index, patch) => updateProject({ ...project, tags: project.tags.map((tag, itemIndex) => itemIndex === index ? { ...tag, ...patch } : tag) });
  const moveTag = (sourceIndex, targetIndex) => {
    if (sourceIndex == null || sourceIndex === targetIndex || targetIndex < 0 || targetIndex >= project.tags.length) return;
  ...
  return <div className="panel-scroll">
    <div className="panel-intro"><div><strong>Prompt Tags</strong><small>{project.tags.length} 个结构化标签</small></div><span className="saved-dot"><Icon name="check"/>自动保存</span></div>
    ...
    <div className="category-legend">{CATEGORY_OPTIONS.slice(0, 4).map((category) => <span key={category} className={`cat-${category.toLowerCase()}`}>{CATEGORY_LABELS[category]}<b>{project.tags.filter((tag) => tag.category === category).length}</b></span>)}</div>
    <div className="tag-stack">
      {project.tags.map((tag, index) => <TagCard key={tag.id} tag={tag} index={index} translating={translatingIds.has(tag.id)} dragging={draggingIndex === index} dropTarget={dropIndex === index && draggingIndex !== index} onTranslate={() => translateTagIds([tag.id])} onChange={(patch) => updateTag(index, patch)} onDelete={() => updateProject({ ...project, tags: project.tags.filter((_, itemIndex) => itemIndex !== index) })} onPointerStart={beginPointerDrag} onPointerMove={movePointerDrag} onPointerEnd={endPointerDrag} onKeyboardMove={keyboardMove}/>)}
      {!project.tags.length && <div className="panel-empty"><Icon name="layers"/><strong>Prompt 还没有标签</strong><span>在上方逐个添加，或从含 NovelAI metadata 的图片自动恢复。</span></div>}
    </div>
  </div>;
}
...
function MetadataPanel({ project, updateProject }) {
  ...
  return <div className="panel-scroll metadata-panel">
    ...
    <label className="textarea-label"><span>Negative Prompt</span><textarea value={metadata.negative_prompt || ''} onChange={(event) => update({ negative_prompt: event.target.value })} placeholder="未检测到 Negative Prompt"/></label>
    <details className="raw-metadata"><summary>查看原始 metadata</summary><pre>{metadata.extra_json || '{}'}</pre></details>
  </div>;
}

function Inspector({ tab, setTab, project, updateProject, showToast }) {
  return <aside className="inspector">
    ...
    {tab === 'tags' && <TagsPanel project={project} updateProject={updateProject} showToast={showToast}/>}
...
export default function App() {
  ...
  useEffect(() => {
    studio.loadLibrary().then((items) => {
      const repairedItems = items.map((project) => {
        ...
        if (tags === project.tags) return project;
        const repaired = { ...project, tags, updated_at: new Date().toISOString() };
        studio.updateProject(repaired);
        return repaired;
      });
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => [project.name, ...project.tags.flatMap((tag) => [tag.tag, tag.translation, tag.category, CATEGORY_LABELS[tag.category]])].some((value) => needles.some((needle) => normalizeSearch(value).includes(needle))));
  }, [projects, query]);
  ...
  const updateProject = (nextProject) => {
    const updated = {
      ...nextProject,
      metadata: { ...nextProject.metadata, prompt_raw: formatPrompt(nextProject.tags) },
  ...
  const copyPrompt = async () => {
    await navigator.clipboard.writeText(formatPrompt(activeProject.tags));
    showToast('Prompt 已复制，可直接粘贴到 NovelAI');
  };

  const saveVersion = () => {
    const versionNumber = activeProject.versions.length + 1;
    const previousTags = activeProject.versions[0] ? JSON.parse(activeProject.versions[0].snapshot_json) : [];
    const previousNames = new Set(previousTags.map((tag) => tag.tag));
    const currentNames = new Set(activeProject.tags.map((tag) => tag.tag));
    ...
    const version = {
      ...
      prompt_text: formatPrompt(activeProject.tags),
      snapshot_json: JSON.stringify(activeProject.tags),
  ...
  const restoreVersion = (versionId) => {
    ...
    if (!version) return;
    updateProject({ ...activeProject, tags: JSON.parse(version.snapshot_json) });
