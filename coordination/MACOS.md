...
- Observed: Windows non-GUI checks completed through `189d05b`: `npm ci`, all 164 tests, the production build, and the Lobe UI audit passed. Windows interactive use showed the original `d7f8f98` Tag fix was incomplete because the shared Tooltip Positioner still intercepted the pointer; `902a217` applies transparency to that actual layer and awaits interactive retesting. It also showed that `902a217` depended on the shared Popover's first-open callback, so every grouped card required a second hover after returning from Workbench. `1f1ccf4` removed that callback dependency but retained a forced cover state and local 8-pixel intent threshold; `189d05b` removes both and maps the entry position directly. Windows and macOS interactive retesting is pending.
