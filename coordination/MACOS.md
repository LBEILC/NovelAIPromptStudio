...
- Observed: Windows non-GUI checks completed through `0a24aa1`: `npm ci`, all 167 tests, the production build, and the Lobe UI audit passed. Earlier Windows interactive use verified direct first-hover group scrubbing through `189d05b` with no issues. The new image-first card layout, synchronized member activation, width-dependent filename, and fixed-panel blank-click behavior await Windows and macOS interactive verification.
