...
- Related commit: `fix: only set signing secrets when MAC_CSC_LINK is configured` (see below)
- Action: The `v0.2.3` release workflow passed macOS tests but failed the packaging step with `⨯ /Users/runner/work/NovelAIPromptStudio/NovelAIPromptStudio not a file`. Cause: `env: CSC_LINK: ${{ secrets.MAC_CSC_LINK }}` sets an empty string when the secret is unconfigured; electron-builder treats the defined-but-empty `CSC_LINK` as a relative file path and resolves it against the project root (`builder-util` `loadCscLink`, cscLink.js). Fix: secrets are now written to `$GITHUB_ENV` only when `MAC_CSC_LINK` is non-empty, and `CSC_IDENTITY_AUTO_DISCOVERY: "false"` (the v0.2.1 configuration) is restored on the macOS packaging step. The next tag (`v0.2.4`) re-runs macOS CI. With no credentials configured, the runner must produce unsigned DMG/ZIP/blockmaps/`latest-mac.yml`; with credentials configured, it must sign and notarize.
- Expected: `v0.2.4` macOS job completes and uploads DMG, ZIP, blockmaps, and `latest-mac.yml`; `SHA256SUMS.txt` is generated; GitHub Release is created with the annotated tag notes. Unsigned builds do not attempt to load an empty `CSC_LINK`.
...
- Status: Blocked
- Date/source: 2026-07-27, macOS 26.3.1 arm64
...
- Observed: macOS completed a clean `npm ci`, 100 automated tests, the Lobe UI audit with zero findings, production build, and unsigned arm64 packaging. The package contains DMG, ZIP, both blockmaps, `latest-mac.yml`, and a launchable `.app`; the production dependency audit reports zero findings. Real packaged-window checks passed the hidden-inset title bar/native menu, CJK/spaces/parentheses dialogs and recent-directory reuse, image/ZIP import, multi-tab restore, dirty `Cmd+W` confirmation, `Cmd+Tab`/`Cmd+Shift+Tab`, ordinary text-field paste, Vibe reveal in Finder, gallery group selection versus current-image deletion, recoverable trash/restore, theme switching, and persisted workbench drafts. Verification found and fixed Finder clipboard file URLs being mistaken for metadata-free bitmaps, plus UTF-8 ZIP entry names without the language flag being decoded as CP437; both fixes passed packaged-app retests. Permanent-delete and empty-trash danger controls/confirmations were inspected without executing the irreversible final actions. The release workflow was corrected to build both macOS architectures in one electron-builder invocation, publish DMG/ZIP/blockmaps plus the merged `latest-mac.yml`, use updater-safe artifact names, and consume signing/notarization secrets when configured; a local unsigned multi-arch build confirmed every URL in the merged update manifest resolves to a generated artifact. Release completion remains blocked: no distribution signing/notarization credentials are configured, strict `codesign`/Gatekeeper validation rejects the ad-hoc package, and the already-published GitHub release (`v0.2.1`) returns 404 for `latest-mac.yml`, so signed update download/install and progress require the next release.
