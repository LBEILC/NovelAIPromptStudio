...
## Verify Lobe UI migration foundation

- Status: Pending
- Date/source: 2026-07-19, Windows
- Related commit: `e9be14b`
- Action: Check out `codex/lobe-ui-migration-spike`, run `npm ci`, `npm test`, `npm run build`, and launch the production app. Open appearance and AI settings, switch every segmented option, focus each input, and return to the library. Confirm the main three-column workspace and version rail retain full height. Inspect the native application menu, hidden-inset title bar, Chinese font fallback, focus rings, and control clipping at the minimum and default window sizes.
- Expected: Lobe controls render without hydration or style-order errors, the root theme container fills the window, settings remain readable, the library layout is unchanged, and macOS native menu/title-bar behavior is preserved. The `@emoji-mart/react` React 19 peer warning may appear during install but must not cause a runtime failure.
- Observed: Windows passed 71 automated tests and the production build. Real-window verification covered the library, appearance settings, AI settings, Windows menu suppression, and the full-height theme-root fix. Awaiting macOS install and UI verification.
