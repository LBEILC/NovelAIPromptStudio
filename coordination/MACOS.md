...
- Related commits: `d549f23`, `acff1c9`, `d7f8f98`, `902a217`, `1f1ccf4`, `189d05b`
- Action: On an unlocked macOS session, follow `doc/manual-verification-20260818-gallery-density-hover-preview.md` and `doc/archive/manual-verification-20260818-preview-pass-through-gallery-double-click.md`. Pay particular attention to trackpad and keyboard control of the thumbnail-size slider, toolbar wrapping at the minimum window width, hover delay while moving across many cards, popup collision near every window edge, light/dark and reduced/off motion modes, and restoration of the selected density after page changes and a full restart. Move the pointer directly onto Gallery cards, Prompt Tags, and Workbench tabs visually covered by their hover previews. Verify the corrected Tag shared Positioner no longer blocks underlying Tags. Click Gallery blank space in fixed and floating modes. Switch from Gallery to Workbench and back, then enter at least three different grouped cards from different horizontal positions on the first hover of each card and scrub across them before double-clicking a standalone card and a grouped card with a known cover.
- Expected: The slider continuously changes the image-first grid between comfortable, compact, and overview density without clipping selection or grouped-card indicators; Gallery, Tag, and Workbench-tab hover previews preserve their content and remain transparent to pointer hit testing over underlying targets. Gallery blank-space clicks collapse and persist the preview panel state. After returning from Workbench, every grouped card immediately previews the member corresponding to the pointer's entry position on its first hover, with no forced cover frame, movement threshold, or second hover. Subsequent horizontal movement keeps the preview and position feedback synchronized, and never changes which cover a double-click opens. Hover layers never block click, selection, context menus, or drag behavior.
- Observed: Windows non-GUI checks completed through `189d05b`: `npm ci`, all 164 tests, the production build, and the Lobe UI audit passed. Windows interactive use showed the original `d7f8f98` Tag fix was incomplete because the shared Tooltip Positioner still intercepted the pointer; `902a217` applies transparency to that actual layer. It also showed that `902a217` depended on the shared Popover's first-open callback, so every grouped card required a second hover after returning from Workbench. `1f1ccf4` removed that callback dependency but retained a forced cover state and local 8-pixel intent threshold; `189d05b` removes both and maps the entry position directly. On 2026-08-19, the user verified on Windows that entering grouped cards from different horizontal positions immediately showed the corresponding member and that multiple first hovers after returning from Workbench all scrubbed correctly, with no issues found. macOS interactive verification remains pending.
